            ...
            <div className="p-8 md:p-12 overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
                {otherProjects.map((project, idx) => (
                  <div 
                    key={idx} 
                    onClick={() => project.caseStudyId && setActiveCaseStudyId(project.caseStudyId)}
                    className={`bg-white rounded-2xl overflow-hidden shadow-sm border border-cloud/30 flex flex-col group ${project.caseStudyId ? 'cursor-pointer hover:shadow-md transition-shadow' : ''}`}
                  >
                    {project.image && (
                    ...
                    <div className="p-6 flex flex-col flex-grow">
                      <h3 className="text-xl font-semibold text-headline mb-3 flex items-center gap-2">
                        {project.title}
                        {project.caseStudyId && <span className="text-xs bg-cta/10 text-cta px-2 py-1 rounded-full font-medium">Case Study</span>}
                      </h3>
                      ...
                      {project.link && (
                        <a 
                          ...
                          onClick={(e) => e.stopPropagation()}
                          className="inline-flex items-center gap-2 text-sm font-medium text-cta hover:text-headline transition-colors mt-auto w-fit"
                        >
                          {project.linkText || 'View Project'} <ArrowRight className="w-4 h-4" />
                        </a>
                      )}
                    </div>
                  </div>
